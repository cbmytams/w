...
export function LeadsDirectory({ leads, onOpenLead }: LeadsDirectoryProps) {
    ...
    const [sortBy, setSortBy] = useState<(typeof SORT_OPTIONS)[number]['id']>('interest');
        ...
        <div className="h-full flex flex-col gap-6">
            ...
            <div className="glass-panel p-4 rounded-2xl border border-white/10 flex flex-col lg:flex-row lg:items-center gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-400">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <select
                            value={sortBy}
                            onChange={(event) => setSortBy(event.target.value as (typeof SORT_OPTIONS)[number]['id'])}
                            className="bg-white/5 border border-white/10 rounded-full px-3 py-2 text-xs text-white"
                        >
            ...
            <div className="flex-1 overflow-hidden rounded-2xl border border-white/5 bg-white/[0.02]">
                ...
                <div className="max-h-[520px] overflow-y-auto custom-scrollbar">
                    {filtered.length === 0 && (
                        <div className="p-8 text-sm text-zinc-500">Aucun profil trouvé.</div>
                    )}
                    {filtered.map((lead) => {
                        ...
                        return (
                            ...
                            >
                                <div>
                                    <div className="text-sm font-semibold text-white">{lead.name || 'Anonymous'}</div>
                                    <div className="text-xs text-zinc-500">{lead.email || 'Email inconnu'}</div>
                                    <div className="text-[10px] text-zinc-600">{formatLeadDate(lead.date)}</div>
                                </div>
                                <div className="text-xs text-zinc-300">{lead.level}</div>
                                <div className={`text-[10px] px-2 py-1 rounded-full border w-fit ${interestTier.color}`}>
                                    {interest}
                                </div>
                                ...
                                <div className="text-xs text-zinc-400">{STATUS_LABELS[lead.status]}</div>
