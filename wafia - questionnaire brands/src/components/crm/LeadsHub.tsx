...
export function LeadsHub({ onOpenLead }: LeadsHubProps) {
    ...
    const [sortBy, setSortBy] = useState<'interest' | 'score' | 'recent'>('interest');
    const queue = {
        ...
        <div className="h-full flex flex-col gap-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div className="flex items-center gap-4">
                    <div className="glass-panel px-4 py-3 rounded-2xl border border-white/10">
                        <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-mono">Total</div>
                        <div className="text-xl font-bold text-white">{metrics.totalLeads}</div>
                    </div>
                    <div className="glass-panel px-4 py-3 rounded-2xl border border-white/10">
                        <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-mono">Actifs</div>
                        <div className="text-xl font-bold text-white">{metrics.activeLeads}</div>
                    </div>
                    <div className="glass-panel px-4 py-3 rounded-2xl border border-white/10">
                        <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-mono">Qualifiés</div>
                        <div className="text-xl font-bold text-white">{leads.filter(l => l.status === 'qualified').length}</div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-3 py-2 text-xs text-zinc-400">
                        ...
                        <select
                            value={sortBy}
                            onChange={(event) => setSortBy(event.target.value as 'interest' | 'score' | 'recent')}
                    ...
                    <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-full p-1">
                        <button
                            ...
                            className={cn(
                                ...
                                view === 'pipeline' ? "bg-white text-black" : "text-zinc-400 hover:text-white"
                            )}
                        >
                        ...
                        <button
                            ...
                            className={cn(
                                "flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest transition-colors",
                                view === 'directory' ? "bg-white text-black" : "text-zinc-400 hover:text-white"
                            )}
                        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="glass-panel px-4 py-3 rounded-2xl border border-red-500/20 bg-red-500/5">
                    <div className="text-[10px] uppercase tracking-widest text-red-300/80 font-mono">Queue P1</div>
                    <div className="text-xl font-bold text-white">{queue.p1}</div>
                    <div className="text-xs text-zinc-400">Leads prioritaires à traiter</div>
                </div>
                <div className="glass-panel px-4 py-3 rounded-2xl border border-amber-500/20 bg-amber-500/5">
                    <div className="text-[10px] uppercase tracking-widest text-amber-300/80 font-mono">Queue P2</div>
                ...
                <div className="glass-panel px-4 py-3 rounded-2xl border border-zinc-500/20 bg-white/5">
                    <div className="text-[10px] uppercase tracking-widest text-zinc-400 font-mono">SLA en retard</div>
