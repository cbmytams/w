...
const CustomTick = ({ x, y, payload }: CustomTickProps) => {
    const pillarKey = PILLAR_ORDER.find(key => PILLARS[key].label === payload.value);
    const color = pillarKey ? PILLARS[pillarKey].color : '#999';

    return (
        <g transform={`translate(${x},${y})`}>
            <text x={0} y={0} dy={4} textAnchor="middle" fill={color} fontSize={11} fontWeight={700} letterSpacing="0.05em" style={{ textTransform: 'uppercase' }}>
                {payload.value}
            </text>
        </g>
    );
        ...
        <div className="w-full aspect-square max-w-lg mx-auto relative group">
            ...
            <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="70%" data={data}>
                    ...
                    <PolarAngleAxis dataKey="pillar" tick={<CustomTick x={0} y={0} payload={{ value: '' }} />} tickLine={false} />
                    ...
                    <Radar
                        ...
                        activeDot={{ fill: '#fff', strokeWidth: 4, stroke: 'var(--heat-start)', r: 6 }}
                    />
                    <Tooltip
                    ...
                    <defs>
                        ...
                        <linearGradient id="heatGradientFill" x1="0%" y1="0%" x2="100%" y2="100%">
