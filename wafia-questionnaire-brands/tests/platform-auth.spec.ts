...
test.describe("platform auth flow", () => {
  test.skip(!platformBaseUrl, "Set PLATFORM_E2E_BASE_URL to run platform auth E2E tests.");

  test("rejects invalid credentials", async ({ request }) => {
    const response = await request.post(`${platformBaseUrl}/api/v1/admin/session`, {
      headers: {
  ...
  test("creates and validates a real admin session", async ({ page }) => {
    test.skip(!adminUsername || !adminPassword, "Set PLATFORM_E2E_ADMIN_USERNAME and PLATFORM_E2E_ADMIN_PASSWORD.");
    ...
    const sessionResponse = await page.request.get(`${platformBaseUrl}/api/v1/admin/session`, {
