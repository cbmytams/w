...
const sanitizeAnswers = (answers: Lead['answers'] | undefined): Record<string, unknown> => {
    if (!answers) return {};
    return Object.entries(answers).reduce<Record<string, unknown>>((acc, [key, value]) => {
...
export const upsertLocalLead = (update: LeadUpdate): Lead[] => {
    ...
    const index = leads.findIndex(
        (lead) => lead.id === update.id || (update.session_id && lead.session_id === update.session_id)
    );
...
export const updateLocalLeadStatus = (id: string, status: Lead['status']): Lead[] => {
    const leads = getLocalLeads();
