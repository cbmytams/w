...
export const getLeadProgress = (lead: Lead) => {
  ...
  const pillar =
    question?.category && question.category !== "QUICK_LEAD"
      ? question.category
      : null;
...
const getSocialPresenceScore = (lead: Lead) => {
  ...
  const count = socialKeys.reduce(
    (acc, key) => (answers[key] ? acc + 1 : acc),
    0
  );
...
const getProfileScore = (lead: Lead) => {
  ...
  if (
    typeof answers.talent_domains === "string" &&
    answers.talent_domains.trim().length > 0
  )
    score += 10;
  if (
    typeof answers.talent_city === "string" &&
    answers.talent_city.trim().length > 0
  )
    score += 6;
  if (
    typeof answers.talent_phone === "string" &&
    answers.talent_phone.trim().length > 0
  )
    score += 8;
  if (answers.talent_has_agency === "yes") score += 6;
  if (
    typeof answers.talent_agency_name === "string" &&
    answers.talent_agency_name.trim().length > 0
  )
    score += 4;
...
export const getLeadInterestScore = (lead: Lead) => {
  ...
  if (!baseScore && Object.keys(answers).length > 0) {
    ...
    const avg =
      PILLAR_ORDER.reduce((acc, pillar) => acc + scores[pillar], 0) /
      PILLAR_ORDER.length;
...
export const getPillarLabel = (pillar?: string | null) => {
  ...
  return (
    (PILLARS as Record<string, { label: string }>)[pillar]?.label ||
