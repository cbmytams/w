...
import { PILLARS, PILLAR_ORDER, INITIAL_SCORES, LEVEL_THRESHOLDS } from '../constants';
    ...
    for (const [questionId, answer] of Object.entries(answers)) {
        ...
        switch (question.type) {
            ...
            case 'scale': {
                ...
                const directPillar = category in scores ? (category as PillarKey) : null;
                const mappedPillar = directPillar ?? SECTION_TO_PILLAR[question.category] ?? null;
...
function scoreMaturity(answers: Answers, scores: Scores): number {
    ...
    const measurementBonus = measurement ? Math.min(measurement.length * 8, 30) : 0;

    // Penalty for 'nothing' or 'unknown' in measurement
    const hasBadMeasurement = measurement?.some(m => m === 'nothing' || m === 'unknown');
    const measurementPenalty = hasBadMeasurement ? -20 : 0;

    return clampScore(avg * 0.5 + channelBonus + measurementBonus + measurementPenalty);
}
...
export function determineLevel(overallScore: number): BrandDiagnosticResult['level'] {
...
export function generateRecommendations(
    ...
    switch (weakness) {
        case 'STRATEGY':
            recommendations.push('Définir une stratégie brand content structurée avec des objectifs SMART');
            if (scores.STRATEGY < 40)
                recommendations.push('Réaliser un audit de positionnement et benchmark concurrentiel');
            break;
        case 'CONTENT':
            recommendations.push('Structurer une production de contenus régulière avec des créateurs alignés');
            if (scores.CONTENT < 40)
                recommendations.push('Investir dans du UGC professionnel pour alimenter votre mix créatif');
            break;
        case 'ACTIVATION':
            recommendations.push('Diversifier vos leviers d\'activation (paid, influence, SEO)');
            if (scores.ACTIVATION < 40)
                recommendations.push('Démarrer par du social ads + whitelisting pour des résultats rapides');
            break;
        case 'DATA':
            recommendations.push('Mettre en place un tracking end-to-end fiable (pixels, UTMs, attribution)');
            if (scores.DATA < 40)
                recommendations.push('Implémenter un dashboard de reporting unifié pour piloter les performances');
            break;
        case 'ORGANIZATION':
            recommendations.push('Structurer les workflows marketing avec des process de validation clairs');
            if (scores.ORGANIZATION < 40)
                recommendations.push('Envisager un accompagnement agence full-service pour combler les gaps d\'équipe');
            break;
    }
    ...
    switch (northStar) {
        case 'awareness':
            recommendations.push('Activer une stratégie multi-créateurs pour maximiser le reach organique');
            break;
        case 'traffic':
            recommendations.push('Combiner UGC ads + SEO content pour générer du trafic qualifié à fort ROI');
            break;
        case 'conversion':
            recommendations.push('Optimiser votre funnel avec du retargeting dynamique et des UGC ads performants');
            break;
        case 'retention':
            recommendations.push('Lancer un programme ambassadeurs structuré avec des créateurs long-terme');
            break;
    }
 ...
 */
function determinePackage(answers: Answers, northStar: NorthStarObjective): PackageSuggestion {
    ...
    const budget = (answers['f_budget_global'] || answers['ql_budget'] || 'undefined') as string;
    ...
    const packages: Record<PackageTier, PackageSuggestion> = {
        starter: {
            tier: 'starter',
            label: 'Starter',
            summary: 'Premiers pas avec l\'influence marketing — Tests et apprentissage',
            ...
            objective: northStar === 'awareness' ? 'Gagner en visibilité' :
                northStar === 'traffic' ? 'Premiers flux de visiteurs' :
                    northStar === 'conversion' ? 'Tester les premiers leviers de conversion' :
        ...
        growth: {
            ...
            objective: northStar === 'awareness' ? 'Installer la marque durablement' :
                northStar === 'traffic' ? 'Scaler l\'acquisition' :
                    northStar === 'conversion' ? 'Optimiser le ROAS et baisser le CAC' :
        ...
        scale: {
            ...
            objective: northStar === 'awareness' ? 'Dominer le Share of Voice' :
                northStar === 'traffic' ? 'Trafic qualifié à grande échelle' :
                    northStar === 'conversion' ? 'Machine de conversion optimisée' :
        ...
        enterprise: {
            ...
            duration: '12+ mois',
            objective: 'Transformation marketing complète avec mesure d\'impact business',
        },
    };
 ...
 */
function generateServiceRecommendations(answers: Answers, northStar: NorthStarObjective): ServiceRecommendation[] {
    ...
    if (audit === 'full' || audit === 'targeted') {
        ...
        services[0].reason = audit === 'full' ? 'Audit complet demandé' : 'Audit ciblé souhaité';
    }

    // UGC if production needs include ugc or short_video
    const production = answers['e_production'] as string[] | undefined;
    if (production?.some(p => ['ugc', 'short_video', 'ad_assets'].includes(p))) {
    ...
    const influenceTypes = answers['e_influence_type'] as string[] | undefined;
    if (influenceTypes && influenceTypes.length > 0 && !influenceTypes.includes('advise')) {
    ...
    if (mediaBuying === 'full' || mediaBuying === 'supervised') {
        ...
        services[3].reason = mediaBuying === 'full' ? 'Délégation complète souhaitée' : 'Accompagnement supervisé demandé';
    }

    // Community management (for retention)
    if (northStar === 'retention') {
        services[4].included = true;
        services[4].reason = 'Objectif fidélisation → community management recommandé';
    }
...
export function generateDiagnosticResult(
    ...
    const northStar: NorthStarObjective = (calibration.northStar || answers['b_north_star'] || answers['ql_objective'] || 'unknown') as NorthStarObjective;
