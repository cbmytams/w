...
const MOCK_LEADS: Lead[] = [
    { id: '1', name: 'Sophie Martin', email: 'sophie.m@creator.com', score: 78, level: 'Mature', date: new Date(Date.now() - 7200000).toISOString(), status: 'contacted' },
    { id: '2', name: 'Thomas Dubreuil', email: 'tom.d@gmail.com', score: 45, level: 'Growth', date: new Date(Date.now() - 18000000).toISOString(), status: 'new' },
    { id: '3', name: 'Léa Dubois', email: 'lea@studio.fr', score: 92, level: 'Expert', date: new Date(Date.now() - 86400000).toISOString(), status: 'signed' },
];

const FALLBACK_ALLOWED = import.meta.env.DEV && import.meta.env.VITE_ALLOW_LOCAL_ADMIN_FALLBACK === 'true';
...
function readFiltersFromUrl(): DashboardFilters {
    ...
    return {
        ...
        source: (search.get('source') as DashboardFilters['source']) || defaults.source,
...
export function AdminProvider({ children }: { children: ReactNode }) {
    ...
    const fetchDashboardData = useCallback(async (activeFilters: DashboardFilters) => {
        setLoading(true);
        try {
            const query = filtersToQueryString(activeFilters);

            const [sessionPayload, overviewPayload, funnelPayload, leadsPayload] = await Promise.all([
                fetchJson<{ user: AdminSessionUser }>('/api/v1/admin/session'),
                fetchJson<{ kpis: KpiCard[] }>(`/api/v1/dashboard/overview?${query}`),
                fetchJson<{ steps: FunnelStep[] }>(`/api/v1/dashboard/funnel?${query}`),
                fetchJson<LeadListPage>(`/api/v1/dashboard/leads?limit=100&${query}`),
            ]);
            ...
            try {
                const auditPayload = await fetchJson<{ events: AuditEvent[] }>(`/api/v1/dashboard/audit?${query}`);
    ...
    useEffect(() => {
        ...
        return () => window.removeEventListener('wafia-local-leads-updated', handler);
    }, [loadLocalFallback]);

    const metrics = useMemo(() => {
        const byId = new Map(kpis.map((kpi) => [kpi.id, kpi]));
        const totalLeads = Math.round(byId.get('leads_incoming')?.value ?? leads.length);
        const conversionRate = Math.round(byId.get('conversion_to_interview_rate')?.value ?? 0);
        const activeLeads = leads.filter((lead) => lead.status === 'new' || lead.status === 'contacted' || lead.status === 'qualified').length;
        const avgScore = Math.round(leads.reduce((acc, lead) => acc + lead.score, 0) / (leads.length || 1));
    ...
    const analytics = useMemo(() => {
        ...
        return {
            velocity: Array.from(velocityMap.entries()).map(([date, value]) => ({ date, value })),
            funnel,
            dropOffs
        };
    }, [funnelSteps, leads]);
