...
const LOCAL_FALLBACK_ALLOWED =
  import.meta.env.DEV &&
  import.meta.env.VITE_ALLOW_LOCAL_QUESTION_FALLBACK === "true";
...
export function DiagnosticProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const initData = async () => {
      setIsLoading(true);
      try {
        const payload = await fetchJson<{ questions?: Question[] }>(
  ...
  const resetToDefaults = async () => {
    if (
      !confirm(
        "Are you sure you want to reset all questions to default? This cannot be undone."
      )
    ) {
      return;
    }

    await replaceQuestionSet(
      ALL_QUESTIONS,
      "Impossible de réinitialiser les questions."
    );
...
export function useDiagnosticData() {
  ...
  if (context === undefined) {
    throw new Error(
      "useDiagnosticData must be used within a DiagnosticProvider"
    );
  }
  return context;
}
