...
export function QuestionCard({ question, answer, onAnswer, calibration, onSubmit, questionNumber }: QuestionCardProps) {
    ...
    const scaleValue = typeof answer === 'number'
        ? answer
        : Math.floor(((question.min || 1) + (question.max || 10)) / 2);

    const processedQuestion = replaceTemplateVariables(question.question, calibration);
    const processedSubtitle = replaceTemplateVariables(question.subtitle, calibration);
    const processedPlaceholder = replaceTemplateVariables(question.placeholder, calibration);
    ...
    const renderInput = () => {
        switch (question.type) {
            case 'single': {
                ...
                const layoutClass = optionCount >= 3
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="absolute inset-0 opacity-0 group-hover:opacity-10 pointer-events-none transition-opacity duration-500"
                                        style={{ background: `radial-gradient(400px circle at ${mousePosition.x}px ${mousePosition.y}px, var(--heat-start), transparent 40%)` }} />

                                    <div className="relative z-10 flex items-start gap-4">
                                        {option.emoji && <span className="text-2xl filter grayscale group-hover:grayscale-0 transition-all">{option.emoji}</span>}
                                        <div className="flex-1">
                                            <div
                                                id={`option-label-${option.id}`}
                                                className={cn("text-lg font-bold leading-tight mb-1 transition-colors", isSelected ? "text-white" : "text-zinc-300 group-hover:text-white")}
                                            >
                                            ...
                                            {option.description && (
                                                <div className="text-sm text-zinc-500 group-hover:text-zinc-400 leading-snug">
                                                    {replaceTemplateVariables(option.description, calibration)}
                                                </div>
                                            )}
                                        </div>
                                        <div className={cn("w-7 h-7 sm:w-6 sm:h-6 shrink-0 rounded-full border flex items-center justify-center transition-all", isSelected ? "bg-[var(--heat-start)] border-[var(--heat-start)] text-white" : "border-zinc-700")}>
                                            {isSelected && <Check className="w-3.5 h-3.5 sm:w-3 sm:h-3" strokeWidth={4} />}
                                        </div>
                                    </div>
                                </motion.button>
                            );
                        ...
                        {question.options?.map((option, index) => {
                            ...
                            return (
                                <motion.button
                                    ...
                                    transition={{ delay: index * STAGGER_DELAY }}
                                    onClick={() => {
                                        const newSelection = isSelected ? selectedIds.filter(id => id !== option.id) : [...selectedIds, option.id];
                                ...
                                >
                                    <span className="text-2xl shrink-0 leading-none mt-1">{option.emoji}</span>
                                    <div className="flex-1 font-semibold text-lg leading-tight text-left">
                                        {replaceTemplateVariables(option.label, calibration)}
                                    </div>
                                    <div className={cn("w-7 h-7 sm:w-6 sm:h-6 shrink-0 rounded-md border flex items-center justify-center transition-all mt-0.5", isSelected ? "bg-[var(--heat-end)] border-[var(--heat-end)] text-white" : "border-zinc-700")}>
            ...
            case 'scale': {
                ...
                return (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full flex flex-col items-center py-4 sm:py-8">
                        ...
                        <div className="relative group w-full">
                            ...
                            <div className="mt-4 flex items-center justify-between">
                                ...
                                <p className="text-xs text-zinc-600">
                                    {typeof answer === 'string' ? answer.length : 0}/{MAX_TEXT_LENGTH} caractères
                                </p>
                            </div>
                        </div>
                    </motion.div>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full">
                        <div className="relative group w-full">
                            <input
                                ...
                                aria-label={processedQuestion}
                                autoComplete={question.type === 'email' ? 'email' : question.type === 'tel' ? 'tel' : 'url'}
                    ...
                    <div className="flex-1 min-w-0 md:sticky md:top-20">
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                            <span className="sr-only" aria-live="polite">
                                Question {questionNumber || ''} sur {/* total will be added via context */}
                            </span>
