...
import { X, Save, Trash2, Plus, AlertCircle, ArrowRight, Target } from 'lucide-react';
import { useDiagnosticData } from '../context/DiagnosticContext';
import type { Question, QuestionType, BrandSection, BrandPillarKey, Condition, ConditionOperator, Impact, Option } from '../types';
...
export function Inspector({ questionId, onClose }: InspectorProps) {
    const { questions, updateQuestion, deleteQuestion, pillars } = useDiagnosticData();
    ...
    const handleDelete = () => {
        if (confirm('⚠️ ATTENTION: Supprimer cette question peut casser la logique du diagnostic. Êtes-vous sûr ?')) {
    ...
    const addImpact = (optionIndex: number) => {
        ...
        const newOptions = [...draft.options];
        newOptions[optionIndex].impacts = [...newOptions[optionIndex].impacts, newImpact];
        updateDraft({ options: newOptions });
    };

    const updateImpact = (optionIndex: number, impactIndex: number, update: Partial<Impact>) => {
        if (!draft?.options) return;
        const newOptions = [...draft.options];
        newOptions[optionIndex].impacts[impactIndex] = { ...newOptions[optionIndex].impacts[impactIndex], ...update };
        updateDraft({ options: newOptions });
    };
    ...
    const removeImpact = (optionIndex: number, impactIndex: number) => {
        if (!draft?.options) return;
        const newOptions = [...draft.options];
        newOptions[optionIndex].impacts = newOptions[optionIndex].impacts.filter((_, i) => i !== impactIndex);
        updateDraft({ options: newOptions });
    };
        ...
        >
            ...
            <div className="h-20 border-b border-white/5 flex items-center justify-between px-6 bg-[#0a0a0a] shrink-0 relative overflow-hidden">
                ...
                <div
                    ...
                    style={{ backgroundColor: draft.category === 'QUICK_LEAD' ? '#ffffff' : (pillars as Record<string, { color: string }>)[draft.category]?.color || '#ffffff' }}
                />

                <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-1">
                        <span className={`text-[10px] font-mono font-bold uppercase tracking-widest px-2 py-0.5 rounded border ${draft.category === 'QUICK_LEAD'
                            ? 'bg-zinc-800 border-zinc-700 text-zinc-400'
                            : 'bg-white/5 border-white/10 text-white'
                            }`}
                            style={{ borderColor: draft.category !== 'QUICK_LEAD' ? (pillars as Record<string, { color: string }>)[draft.category]?.color : undefined }}
                        >
                            {draft.category}
                        </span>
                ...
                <div className="flex items-center gap-2 relative z-10">
                    ...
                    <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 text-zinc-500 hover:text-white transition-colors">
                        <X className="w-5 h-5" />
                    </button>
                </div>
            </div>
                ...
                <div className="p-6 space-y-10">
                    ...
                    <section className="space-y-4">
                        ...
                        <div className="space-y-4">
                            <div className="relative group">
                                <label className="block text-[10px] uppercase text-zinc-600 font-bold mb-1.5 transition-colors group-focus-within:text-[var(--heat-start)]">The Question</label>
                                <textarea
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    <label className="block text-[10px] uppercase text-zinc-600 font-bold mb-1.5">Context / Subtitle</label>
                                    <input
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-[10px] uppercase text-zinc-600 font-bold mb-1.5">Input Type</label>
                                        <select
                                            value={draft.type}
                                            onChange={e => updateDraft({ type: e.target.value as QuestionType })}
                                    ...
                                    <div>
                                        <label className="block text-[10px] uppercase text-zinc-600 font-bold mb-1.5">Pillar</label>
                                        <select
                                            value={draft.category}
                                            onChange={e => updateDraft({ category: e.target.value as BrandSection })}
                                            className="w-full bg-[#0f0f12] border border-white/10 rounded-lg p-2 text-sm text-white focus:border-white/20 outline-none hover:bg-white/5 cursor-pointer transition-colors"
                                        >
                    ...
                    <section className="space-y-4">
                        <div className="flex items-center justify-between">
                            <SectionHeader icon={AlertCircle} title="Conditions d'Affichage" />
                            <button onClick={addCondition} className="text-[10px] uppercase font-bold text-zinc-500 hover:text-[var(--heat-start)] flex items-center gap-1 transition-colors px-2 py-1 hover:bg-white/5 rounded">
                        ...
                        <div className="space-y-2">
                            ...
                            {draft.conditions?.map((cond, idx) => (
                                <div key={idx} className="flex items-center gap-3 p-3 bg-[#0f0f12] border border-white/5 rounded-xl group hover:border-white/10 transition-colors">
                                    <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">AFFICHER SI</div>
                    ...
                    {(draft.type === 'single' || draft.type === 'multiple') && (
                        <section className="space-y-4 pt-4 border-t border-white/5">
                            <div className="flex items-center justify-between">
                                ...
                                <button onClick={addOption} className="text-[10px] uppercase font-bold text-zinc-500 hover:text-[var(--heat-start)] flex items-center gap-1 transition-colors px-2 py-1 hover:bg-white/5 rounded">
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                {draft.options?.map((opt, optIdx) => (
                                    <div key={optIdx} className="group relative overflow-hidden bg-[#0f0f12] border border-white/5 rounded-xl hover:border-white/10 hover:bg-white/[0.02] transition-all duration-300">
                                        {/* OPTION HEADER */}
                                        <div className="p-4 flex items-start gap-3">
                                            <div className="w-10 h-10 flex items-center justify-center bg-white/5 rounded-lg border border-white/5 group-hover:border-[var(--heat-start)]/50 transition-colors">
                                                <input
                                                    value={opt.emoji || ''}
                                                    onChange={e => updateOption(optIdx, { emoji: e.target.value })}
                                            ...
                                            <div className="flex-1 space-y-2">
                                                ...
                                                <div className="flex gap-2">
                                                    <div className="bg-black/40 rounded px-2 py-0.5 text-[10px] text-zinc-500 font-mono border border-white/5 flex items-center gap-2">
                                                        ...
                                                        <input
                                                            value={opt.id}
                                                            onChange={e => updateOption(optIdx, { id: e.target.value })}
                                            ...
                                            <button
                                                onClick={() => {
                                                    const newOptions = draft.options?.filter((_, i) => i !== optIdx);
                                                    updateDraft({ options: newOptions });
                                                }}
                                        ...
                                        <div className="bg-black/20 border-t border-white/5 p-3">
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <button onClick={() => addImpact(optIdx)} className="text-[10px] text-[var(--heat-end)] hover:text-white transition-colors">+ Ajouter Score</button>
                                            </div>

                                            <div className="space-y-2">
                                                {opt.impacts.length === 0 && (
                                                    <div className="text-[10px] text-zinc-700 italic px-2">Neutre (Aucun impact)</div>
                                                )}
                                                {opt.impacts.map((imp, impIdx) => (
                                                    <div key={impIdx} className="flex items-center gap-2 bg-white/5 rounded pl-1 pr-2 py-1">
                                                        {/* Pillar Color Dot */}
                                                        <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: pillars[imp.pillar]?.color || '#fff' }} />

                                                        <select
                                                            value={imp.pillar}
                                                            onChange={e => updateImpact(optIdx, impIdx, { pillar: e.target.value as BrandPillarKey })}
                                                        ...
                                                        <input
                                                            type="number"
                                                            value={imp.weight}
                                                            onChange={e => updateImpact(optIdx, impIdx, { weight: parseInt(e.target.value) || 0 })}
