    ...
    return (
        ...
        >
            ...
            <div className="h-20 border-b border-white/5 flex items-center justify-between px-6 bg-[#0a0a0a] shrink-0">
                <div>
                    <div className="text-xs text-zinc-500 uppercase tracking-widest font-mono">Lead Details</div>
                    <div className="text-lg font-bold text-white tracking-tight">{lead.name || 'Anonymous'}</div>
                </div>
                <button
                    onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
                <div className="space-y-4">
                    <div className="flex items-center gap-3">
                        <span className={`px-3 py-1 rounded-full text-[10px] uppercase font-bold tracking-widest border ${STATUS_STYLES[lead.status]}`}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 rounded-xl border border-white/5 bg-white/[0.02]">
                            <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-mono">Score</div>
                            <div className="text-2xl font-bold text-white">{lead.score}</div>
                        </div>
                        <div className="p-4 rounded-xl border border-white/5 bg-white/[0.02]">
                            <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-mono">Niveau</div>
                        ...
                        <div className="p-4 rounded-xl border border-white/5 bg-white/[0.02]">
                            <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-mono">Intérêt</div>
                            <div className={`text-sm font-semibold px-2 py-1 rounded-full border w-fit ${interestTier.color}`}>
                                {interest} • {interestTier.label}
                            </div>
                        </div>
                        <div className="p-4 rounded-xl border border-white/5 bg-white/[0.02] col-span-2">
                            <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-mono">Progression</div>
                            <div className="mt-2 flex items-center gap-3">
                                <div className="flex-1 h-2 rounded-full bg-white/5 overflow-hidden">
                                    <div className="h-full bg-gradient-to-r from-[var(--heat-start)] to-[var(--heat-end)]" style={{ width: `${completion}%` }} />
                                </div>
                        ...
                        {tags.length > 0 && (
                            <div className="p-4 rounded-xl border border-white/5 bg-white/[0.02] col-span-2">
                                <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-mono">Tags</div>
                                <div className="mt-2 flex flex-wrap gap-2">
                                    {tags.map((tag) => (
                                        <span key={tag} className="text-[10px] px-2 py-1 rounded-full bg-white/5 text-zinc-300">
                                            {tag}
                                        </span>
                                    ))}
                                </div>
                ...
                <div className="space-y-3">
                    <div className="text-xs uppercase tracking-widest text-zinc-500 font-mono">Contact</div>
                    <div className="flex flex-col gap-3">
                        <div className="p-4 rounded-xl border border-white/5 bg-white/[0.02]">
                            <div className="text-xs text-zinc-500">Email</div>
                            <div className="text-sm text-white break-all">{displayEmail || 'Non renseigné'}</div>
                ...
                <div className="space-y-3">
                    <div className="text-xs uppercase tracking-widest text-zinc-500 font-mono">Réponses</div>
                    {answers.length === 0 ? (
                        <div className="text-sm text-zinc-500">Aucune réponse disponible.</div>
                    ) : (
                        <div className="space-y-3">
                            {answers.slice(0, 12).map(([key, value]) => (
                                <div key={key} className="p-3 rounded-lg border border-white/5 bg-white/[0.02]">
                                    <div className="text-[10px] uppercase tracking-widest text-zinc-500 font-mono">{key}</div>
                            ...
                            {answers.length > 12 && (
                                <div className="text-xs text-zinc-500">… {answers.length - 12} réponses supplémentaires</div>
                            )}
                        </div>
                    )}
                </div>
