...
import { scoresToRadarData, getLevelLabel, getLevelColor } from '../utils/scoring';
...
const CARD_BASE = "rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md p-6 sm:p-8";

export function BrandResultsSummary({ result, onReset }: BrandResultsSummaryProps) {
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 sm:py-16">
                ...
                >
                    ...
                    <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold text-white tracking-tight leading-tight mb-4">
                        Votre profil
                        <span className="bg-gradient-to-r from-[var(--heat-start)] to-[var(--heat-end)] bg-clip-text text-transparent"> stratégique</span>
                    </h1>
                    ...
                    <motion.div
                        ...
                        style={{ borderColor: levelColor + '40', color: levelColor, backgroundColor: levelColor + '15' }}
                    >
                        <span className="text-sm font-bold tracking-wide">{levelLabel}</span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-12">
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                <div className="text-5xl font-black text-white">{result.leadScore.total}</div>
                        ...
                        <div className="space-y-3">
                            {[
                                { label: 'Budget', value: result.leadScore.breakdown.budget, weight: '30%' },
                                { label: 'Urgence', value: result.leadScore.breakdown.urgency, weight: '20%' },
                                { label: 'Maturité', value: result.leadScore.breakdown.maturity, weight: '15%' },
                                { label: 'Fit Services', value: result.leadScore.breakdown.fit, weight: '20%' },
                                { label: 'Décision', value: result.leadScore.breakdown.decision, weight: '15%' },
                            ].map((item) => (
                                <div key={item.label}>
                                    <div className="flex justify-between text-xs mb-1">
                                        <span className="text-zinc-400">{item.label}</span>
                                        <span className="text-zinc-500 font-mono">{item.value}/100 ({item.weight})</span>
                                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-12">
                    ...
                    >
                        ...
                        <div className="text-2xl font-bold text-white mb-3 capitalize">
                            {result.northStar === 'awareness' ? '🎯 Notoriété' :
                                result.northStar === 'traffic' ? '🚀 Trafic' :
                                    result.northStar === 'conversion' ? '💰 Conversion' :
                                        result.northStar === 'retention' ? '🤝 Fidélisation' :
                                            'Non défini'}
                        </div>
                        ...
                        <div className="space-y-3">
                            {result.services.map((service) => (
                                ...
                                >
                                    ...
                                    <div>
                                        <div className={`text-sm font-semibold ${service.included ? 'text-white' : 'text-zinc-600'}`}>
                                            {service.label}
                                        </div>
                                        {service.included && service.reason && (
                                            <div className="text-xs text-zinc-500 mt-0.5">{service.reason}</div>
                                        )}
                                    </div>
                                </div>
                            ))}
                ...
                >
                    ...
                    <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-8">
                        ...
                        <div className="text-right shrink-0">
                            <div className="text-2xl font-bold text-white">{result.package.priceRange}</div>
                            <div className="text-xs text-zinc-500 mt-1">{result.package.duration}</div>
                        </div>
                    </div>
                    ...
                    <div className="py-3 px-4 rounded-xl bg-white/5 border border-white/5 text-sm text-zinc-300 inline-block">
                        🎯 <span className="font-semibold text-white">{result.package.objective}</span>
                    </div>
                </motion.div>
                ...
                >
                    ...
                    <p className="text-zinc-400 mb-8 max-w-md mx-auto">
                        Notre équipe vous contactera sous 24h avec une proposition personnalisée basée sur votre diagnostic.
                    </p>
                    ...
                    {includedServices.length > 0 && (
                        <div className="mt-10 flex flex-wrap justify-center gap-2">
                            {includedServices.map((s) => (
                                <span key={s.id} className="px-3 py-1 rounded-full text-xs font-medium bg-white/5 border border-white/10 text-zinc-400">
                                    {s.label}
                                </span>
                            ))}
                        </div>
