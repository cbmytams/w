...
export async function requireTenantContext(session: AdminSession): Promise<TenantContextResult> {
  const tenantId = await resolveTenantIdForSession(session.tenantSlug);
  if (!tenantId) {
    return {
      tenantId: null,
      response: jsonApiError("TENANT_MISCONFIGURED", "Tenant misconfigured", { status: 503 })
    };
  }
