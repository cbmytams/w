...
export function canAccessDashboardRole(current: DashboardRole, required: DashboardRole) {
  return DASHBOARD_ROLE_RANK[current] >= DASHBOARD_ROLE_RANK[required];
}

export function mapPlatformRoleToDashboardRole(role?: string | null): DashboardRole {
  switch (role) {
