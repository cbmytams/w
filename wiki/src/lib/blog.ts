...
const mdxFiles = import.meta.glob('/src/content/blog/**/*.{md,mdx}', { query: '?raw', import: 'default', eager: true });
console.log("Vite GLOB found files:", Object.keys(mdxFiles));
    ...
    for (const path in mdxFiles) {
        try {
            ...
            if (typeof rawContent !== 'string') {
                console.error('rawContent is not a string for path:', path, rawContent);
                continue;
            }
            ...
            if (filename.includes('plateforme-')) {
                ...
                category = category === "Article" ? `Plateforme — ${platform.charAt(0).toUpperCase() + platform.slice(1)}` : category;
            ...
            console.error('Error parsing article:', path, e);
        }
    }
