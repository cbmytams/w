    ...
    useEffect(() => {
        if (query.length > 1) {
            ...
            const scored = articles.map(a => {
                ...
                if (a.chapters?.some(c => c.title.toLowerCase().includes(q))) score += 3;
                if (a.chapters?.some(c => c.content.toLowerCase().includes(q))) score += 1;
    ...
    return (
        <AnimatePresence>
            <motion.div
                ...
                style={{ backgroundColor: 'var(--search-bg)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)' }}
            ...
            >
                <motion.div
                    ...
                    style={{ backgroundColor: 'var(--bg-elevated)', borderColor: 'var(--line)' }}
                >
                    <div className="flex items-center px-6 py-2 border-b gap-2" style={{ borderColor: 'var(--line)' }}>
                        <div className="w-12 h-12 flex items-center justify-center shrink-0">
                            <Search className="w-6 h-6" style={{ color: 'var(--ink-secondary)' }} />
                        </div>
                        <input
                            autoFocus
                        ...
                        {results.length > 0 ? (
                            <div className="py-2">
                                {results.map((r) => (
                                    <button
                                        key={r.slug}
                                        onClick={() => { setIsOpen(false); navigate(`/blog/${r.slug}`); }}
                                    ...
                                    >
                                        <div>
                                            <p className="font-serif text-xl transition-all duration-300" style={{ color: 'var(--ink)' }}>{r.title}</p>
                                            <p className="font-mono text-[10px] uppercase tracking-widest mt-2" style={{ color: 'var(--ink-secondary)' }}>{r.category} • {r.time}</p>
                                        </div>
                            ...
                            <div className="py-16 text-center font-mono text-sm max-w-md mx-auto" style={{ color: 'var(--ink-secondary)' }}>
                                Aucun résultat pour « {query} ».
                            </div>
                        ) : (
                            <div className="py-10 text-center font-mono text-sm uppercase tracking-widest border-t" style={{ color: 'var(--ink-muted)', borderColor: 'var(--line)' }}>
