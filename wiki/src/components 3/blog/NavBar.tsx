...
export default function NavBar({ isDeep, isReading, parentLabel, currentTitle, onBack, showUI = true, isDark, onToggleDark }: NavBarProps) {
    ...
    return (
        <motion.nav
            ...
            style={{ backgroundColor: 'var(--nav-bg)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', borderBottom: '1px solid var(--line)' }}
        >
            {isDeep && (
                <motion.button
            ...
            {isReading && currentTitle && (
                ...
                >
                    <span className="font-serif italic text-lg line-clamp-1 block px-4 opacity-80" style={{ color: 'var(--ink)', maskImage: 'linear-gradient(to right, transparent, black 5%, black 95%, transparent)', WebkitMaskImage: 'linear-gradient(to right, transparent, black 5%, black 95%, transparent)' }}>
                        {currentTitle}
                    </span>
                </motion.div>
            )}
