...
export default function ArticleReaderPage() {
    ...
    useEffect(() => {
        ...
        return () => { document.title = "Wiki de l'Influence"; };
    }, [article]);
    ...
    if (!article) return <div className="min-h-screen" style={{ backgroundColor: 'var(--bg)' }} />;

    const progress = article.chapters?.length ? ((chapter + 1) / article.chapters.length) * 100 : 0;

    return (
        <div id="wiki-root" className="min-h-screen font-sans flex flex-col" style={{ backgroundColor: 'var(--bg)', color: 'var(--ink)' }}>

            <NavBar isDeep isReading parentLabel={article.category}
                currentTitle={article.title} onBack={() => navigate(-1)} showUI={showUI}
            ...
            >
                ...
                <header className="mb-16 text-center max-w-3xl mx-auto mt-4">
                    <span className="font-mono text-xs uppercase tracking-[0.2em] mb-6 block" style={{ color: 'var(--ink-secondary)' }}>
                ...
                <div className="flex-1 relative">
                    <AnimatePresence mode="wait" custom={direction}>
                        {article.chapters?.length > 0 ? (
                            ...
                            >
                                <h2 className="font-sans text-xl md:text-2xl font-semibold tracking-tight mb-8" id="chapter-title">
                                    <span className="mr-3 font-mono text-sm md:text-base font-normal" style={{ color: 'var(--ink-secondary)' }}>
                                        {String(chapter + 1).padStart(2, '0')}
                                    </span>
                                    {article.chapters[chapter].title}
                                </h2>
                                <div className="font-serif text-lg md:text-[1.3rem] leading-[1.8] prose-p:mb-6 prose-a:underline prose-strong:font-semibold prose-ul:list-disc prose-ul:pl-6 prose-li:mb-2 prose-h3:font-sans prose-h3:text-lg prose-h3:font-semibold prose-h3:mt-10 prose-h3:mb-4 prose-blockquote:border-l-2 prose-blockquote:pl-6 prose-blockquote:italic"
                            ...
                            <div className="prose prose-lg max-w-none">
                                <p className="font-serif text-xl leading-relaxed" style={{ color: 'var(--prose-body)' }}>
                ...
                {article.chapters?.length > 1 && (
                    ...
                    >
                        ...
                        >
                            <div className="w-12 h-12 rounded-full border flex items-center justify-center group-hover:opacity-60 transition-all shrink-0" style={{ borderColor: 'var(--line)' }}>
                                <span className="text-lg leading-none">←</span>
                            </div>
                            <div className="text-left hidden sm:block min-w-0">
                                <span className="font-mono text-[10px] uppercase tracking-widest block mb-1" style={{ color: 'var(--ink-secondary)' }}>Précédent</span>
                                <span className="font-sans text-sm font-medium line-clamp-1 block">{chapter > 0 ? article.chapters[chapter - 1].title : ''}</span>
                            </div>
                        </button>

                        <div className="font-mono text-xs shrink-0 px-2" style={{ color: 'var(--ink-secondary)' }} aria-live="polite">
                        ...
                        >
                            <div className="text-right hidden sm:block min-w-0">
                                <span className="font-mono text-[10px] uppercase tracking-widest block mb-1" style={{ color: 'var(--ink-secondary)' }}>Suivant</span>
            ...
            {article.chapters?.length > 1 && (
                ...
                >
                    <nav aria-label="Sommaire de l'article" className="flex flex-col h-full">
                        <h4 className="font-mono text-xs uppercase tracking-widest mb-6 shrink-0" style={{ color: 'var(--ink-secondary)' }}>Sommaire</h4>
                        <div className="flex-1 overflow-y-auto pr-4 pb-4" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                            <style>{`
                                nav div::-webkit-scrollbar { display: none; }
                            `}</style>
                            <ul className="space-y-5 border-l relative" style={{ borderColor: 'var(--line)' }}>
                                <motion.div
                                ...
                                {article.chapters.map((c, i) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <button
                                                onClick={() => { setDirection(i > chapter ? 1 : -1); setChapter(i); }}
