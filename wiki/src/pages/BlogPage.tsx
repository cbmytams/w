...
export default function BlogPage() {
    ...
    return (
        <div id="wiki-root" className="min-h-screen font-sans flex flex-col" style={{ backgroundColor: 'var(--bg)', color: 'var(--ink)' }}>
            <NavBar isDeep={false} isReading={false} parentLabel="Index" isDark={isDark} onToggleDark={toggleDark} />
            ...
            <main className="pt-32 px-6 md:px-12 max-w-5xl mx-auto pb-24 flex-1 w-full">
                ...
                >
                    <h1 className="font-serif text-5xl sm:text-6xl md:text-8xl font-medium leading-[0.9] tracking-tight">
                        Wiki de <br />
                        <span className="italic" style={{ color: 'var(--ink-secondary)' }}>l'Influence.</span>
                    </h1>
                </motion.div>

                <div className="mb-12">
                    <TabSwitcher tabs={TABS} active={tab} onChange={v => setTab(v as any)} />
                </div>
                        ...
                        >
                            {items.map((item) => (
                                <motion.div key={item.id}
                                    ...
                                    role="button"
                                    tabIndex={0}
                                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate(`${routePrefix}/${item.id}`); } }}
                                ...
                                >
                                    <div className="transform transition-transform duration-300 ease-out group-hover:translate-x-2">
                                        <span className="font-mono text-xs mb-2 block" style={{ color: 'var(--ink-secondary)' }}>{item.num}</span>
                                        <p className="font-serif text-3xl transition-colors duration-300 group-hover:opacity-60">
                                            {item.label}
                                        </p>
                                    </div>
                                    <div className="w-8 h-8 rounded-full border flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 -translate-x-4 group-hover:translate-x-0" style={{ borderColor: 'var(--line)' }}>
            ...
            <footer className="border-t py-12 px-6 md:px-12 max-w-5xl mx-auto w-full" style={{ borderColor: 'var(--line)' }}>
