...
export function LegalHubContent({ context = "talents" }: LegalHubContentProps) {
  ...
  // Color classes
  const hoverBorderColor = isBrand
    ? "hover:border-orange-500/50"
    : "hover:border-violet-500/50";
  const hoverBgColor = isBrand
    ? "hover:bg-orange-500/5"
    : "hover:bg-violet-500/5";
  const hoverShadowColor = isBrand
    ? "hover:shadow-orange-500/10"
    : "hover:shadow-violet-500/10";
  const iconHoverBg = isBrand
    ? "group-hover:bg-orange-500"
    : "group-hover:bg-violet-500";
  const textHoverColor = isBrand
  ...
  return (
    <div className="space-y-6">
      <p className="text-slate-600 dark:text-slate-400">
        La transparence est la base de notre relation. Retrouvez ci-dessous
        l&apos;ensemble de nos documents légaux et de nos engagements en matière
        de confidentialité.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        >
          ...
          <div>
            <span className="block font-semibold text-slate-900 dark:text-white mb-1">
              Mentions Légales
            </span>
            <span
              className={cn(
                "text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1 transition-colors",
                textHoverColor
              )}
            >
              Consulter{" "}
