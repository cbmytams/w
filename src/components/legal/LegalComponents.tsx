...
export function LegalSection({ title, children }: { title: string, children: ReactNode }) {
    return (
        <motion.section variants={itemVariants} className="relative pt-8 first:pt-0">
            <div className="mb-8">
...
export function LegalCard({ title, value, icon }: { title: string, value: ReactNode, icon?: ReactNode }) {
    return (
        <div className="group relative bg-black/[0.02] dark:bg-white/[0.02] backdrop-blur-[40px] border border-black/[0.05] dark:border-white/[0.05] rounded-2xl p-8 hover:shadow-[0_24px_64px_rgba(0,0,0,0.08)] dark:shadow-none transition-all duration-500 hover:-translate-y-2 ease-[cubic-bezier(0.16,1,0.3,1)]">
            ...
            <div className="relative z-10">
                <div className="text-xs text-slate-500 dark:text-slate-400 font-mono mb-4 uppercase tracking-widest flex justify-between items-start">
                    <span>{title}</span>
                    {icon && <span className="opacity-50 group-hover:opacity-100 transition-opacity duration-300">{icon}</span>}
                </div>
