...
type TalentProfileData =
  (typeof TALENT_TEAM_DATA)[keyof typeof TALENT_TEAM_DATA];
...
function TeamCardTalents({
  ...
  const { disableMotion, viewport, transitionDuration, clampDelay } =
    useRevealViewport();
  const talentData =
    TALENT_TEAM_DATA[member.id as keyof typeof TALENT_TEAM_DATA] ??
    TALENT_TEAM_DATA.sasha;
  const geoConfig =
    GEOGRAPHY_CONFIG[talentData.geo as keyof typeof GEOGRAPHY_CONFIG];
  ...
  return (
    <motion.article
      ...
      transition={
        disableMotion
          ? undefined
          : {
              duration: Math.max(transitionDuration, 0.44),
              delay: clampDelay(index * 0.1),
              ease: EASING.subtle,
            }
      }
    ...
    >
      ...
      <div
        ...
        style={{
          boxShadow: isHovered
            ? `0 20px 50px ${geoConfig.shadowColor}`
            : undefined,
          borderColor: isHovered
            ? geoConfig.shadowColor.replace("0.15", "0.3")
            : undefined,
        }}
      >
        {/* Top Gradient Bar */}
        <motion.div
          ...
          transition={
            disableMotion
              ...
              : {
                  duration: Math.max(transitionDuration, 0.44),
                  delay: clampDelay(0.16 + index * 0.08),
                }
          }
        />
        ...
        <div className="p-8 md:p-10">
          ...
          <div className="flex flex-col sm:flex-row gap-6 sm:items-start mb-6">
              ...
              style={
                {
                  "--tw-ring-color": isHovered
                    ...
                    : "transparent",
                  boxShadow: isHovered
                    ? `0 8px 20px ${geoConfig.shadowColor}`
                    : "none",
            ...
            <div className="flex-1 mt-2">
              ...
              <h3
                className="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white tracking-tight mb-1 transition-colors"
                style={{
                  color: isHovered
                    ? geoConfig.shadowColor.replace("0.15", "1")
                    : undefined,
                }}
              >
                {member.name}
...
export function TeamSectionTalents() {
  ...
  return (
    <section
      id="team"
      className="pt-10 pb-28 lg:pt-20 lg:pb-36 px-4 relative z-10"
    >
      <Container>
