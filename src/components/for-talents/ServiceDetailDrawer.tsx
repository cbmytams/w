...
export function ServiceDetailDrawer({ service, onClose }: ServiceDetailDrawerProps) {
  const ctaHref = buildTalentQuestionnaireHref("for-talents-service-drawer")
  const [mounted, setMounted] = useState(false)
  const [isMobile, setIsMobile] = useState(false)
  const [renderedService, setRenderedService] = useState<Service | null>(null)
  ...
  }, [renderedService, onClose])

  useFocusTrap(dialogRef, isOpen && !isMobile)

  if (!mounted || (!renderedService && !isOpen)) return null
  if (!renderedService || !renderedService.detail) return null

  const detail = renderedService.detail
                  ...
                  {detail.deliverables && detail.deliverables.length > 0 && (
                    <div className="mb-10">
                      ...
                      <ul className="space-y-2.5">
                        {detail.deliverables.map((deliverable, i) => (
                          <li
                            key={i}
