      ...
      <motion.div
        ...
        transition={
          prefersReducedMotion
            ? undefined
            : { duration: 0.38, ease: EASING.smooth }
        }
      ...
      >
        ...
        {STUDIO_PRODUCTIONS.filter((p) => !p.comingSoon).map((prod) => (
          ...
          >
            ...
            <div
              className={cn(
                "absolute inset-0 bg-gradient-to-br opacity-10 transition-opacity duration-700 group-hover:opacity-30",
                prod.gradient
              )}
            />
            ...
            <div className="relative h-full flex flex-col p-8 md:p-12">
              <div className="flex justify-between items-start">
                <motion.div
                  ...
                  className={cn(
                    "w-16 h-16 rounded-2xl flex items-center justify-center text-black shadow-lg bg-gradient-to-br",
                    prod.gradient
                  )}
                >
              ...
              <div className="mt-auto">
                <motion.p
                  ...
                  className="text-xs md:text-sm font-semibold uppercase tracking-[0.2em] text-white/50 mb-3"
                >
                  {prod.tagline}
                </motion.p>
                <motion.h3
        ...
        <div className="md:col-span-1 flex flex-col gap-6 md:gap-8 h-full">
          {STUDIO_PRODUCTIONS.filter((p) => p.comingSoon).map((prod) => (
            ...
            >
              ...
              <div
                className={cn(
                  "absolute inset-0 bg-gradient-to-br opacity-[0.03]",
                  prod.gradient
                )}
              />
      ...
      <AnimatePresence>
        {selectedId && (
          <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center sm:p-6 lg:p-12">
            ...
            {/* Sheet / Window */}
            {STUDIO_PRODUCTIONS.filter((p) => p.id === selectedId).map(
              (prod) => (
                <motion.div
                  layoutId={`card-${prod.id}`}
                  key={prod.id}
                  ...
                  <div className="shrink-0 flex items-center justify-between p-6 sm:p-8 z-20 relative">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <motion.h3
                          layoutId={`title-${prod.id}`}
                          className="text-xl sm:text-2xl font-bold text-white tracking-tight leading-none mb-1"
                        >
                          {prod.name}
                        </motion.h3>
                        <motion.p
                          layoutId={`tagline-${prod.id}`}
                          className="text-[10px] sm:text-[11px] font-semibold text-white/50 tracking-[0.15em] uppercase"
                        >
                          {prod.tagline}
                        </motion.p>
                      </div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 max-w-5xl mx-auto h-full lg:items-center">
                      ...
                      <div className="flex flex-col w-full max-w-[500px] mx-auto lg:mx-0 lg:mr-auto justify-center space-y-10 lg:-mt-10">
                        {/* The Mission */}
                        <div>
                          <h4 className="text-[12px] font-semibold text-white/30 uppercase tracking-[0.2em] mb-4">
                            La Mission
                          </h4>
                        ...
                        {prod.id === "krh" ? (
                          <div className="flex flex-col sm:flex-row gap-3 pt-4">
                            ...
                            <div className="w-full sm:flex-1 relative group cursor-not-allowed">
                              ...
                              >
                                Découvrir le réal
                                <span className="ml-1 text-[10px] font-bold tracking-[0.25em] text-white/40 uppercase bg-white/10 px-2 py-0.5 rounded-full">
                                  Soon
                                </span>
                              </button>
                            </div>
