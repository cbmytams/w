...
import { AlertTriangle, CheckCircle, Save, Database, ServerCrash, Activity } from "lucide-react";
...
export function QuestionnaireHealthPanel({ type }: { type: "TALENTS" | "BRANDS" }) {
    ...
    const statusColor = isOk ? "text-green-500" : data.status === "warning" ? "text-yellow-500" : "text-red-500";
    const StatusIcon = isOk ? CheckCircle : data.status === "warning" ? AlertTriangle : ServerCrash;
                    ...
                    {data.issues.length > 0 ? (
                        <ul className="space-y-3">
                            {data.issues.map((issue, idx) => (
                                <li key={idx} className="p-4 rounded-xl bg-red-500/5 border border-red-500/10 flex items-start gap-4 transition-colors hover:bg-red-500/10">
                        ...
                        <div className="p-8 rounded-xl bg-green-500/5 border border-green-500/10 flex flex-col items-center justify-center text-center">
                            ...
                            <h4 className="text-lg font-medium text-green-400 mb-1">Système Nominal</h4>
                            <p className="text-sm text-green-500/70">Aucune anomalie détectée dans la structure des données. Les correspondances entre la base et les schémas front-end sont parfaites.</p>
                        </div>
                    )}
                </div>
            </div>
