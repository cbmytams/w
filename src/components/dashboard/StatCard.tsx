  ...
  return (
    <div className="p-6 rounded-2xl bg-questionnaire-surface border border-questionnaire-muted glass-panel relative overflow-hidden group transition-all hover:-translate-y-1">
      ...
      <div className="flex items-baseline gap-3 relative z-10">
        ...
        {trend && (
          <span
            className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${trendPositive ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-red-500/10 text-red-500 border border-red-500/20"}`}
          >
            {trend}
          </span>
        )}
      ...
      {description && (
        <p className="mt-3 text-xs text-slate-500 relative z-10 font-medium">
          {description}
        </p>
      )}
    </div>
