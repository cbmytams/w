...
export function HomeClient() {
  ...
  React.useLayoutEffect(() => {
    ...
    const transitionFlag = window.sessionStorage.getItem(
      "wafia-nav-transition"
    );
  ...
  return (
    <div
      id="home-root"
      className="min-h-screen w-full bg-[#050510] flex flex-col relative overflow-hidden selection:bg-brand-primary/30"
    >
      {/* SPACE BACKGROUND */}
      <StaticBackground />
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <span className="font-heading text-white/80 text-sm font-medium tracking-wide group-hover:text-white transition-colors duration-300">
                    {option.label}
                  </span>
                  ...
                  <div
                    className={`absolute inset-0 rounded-full bg-gradient-to-r ${option.gradient} opacity-0 group-hover:opacity-100 blur-xl -z-10 transition-opacity duration-500`}
                  />
                </Link>
              </motion.div>
            );
