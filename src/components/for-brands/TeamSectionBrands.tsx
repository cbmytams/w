...
function TeamCardBrands({
  ...
  const { disableMotion, viewport, transitionDuration, clampDelay } =
    useRevealViewport();
  const brandData =
    BRAND_TEAM_DATA[member.id as keyof typeof BRAND_TEAM_DATA] ??
    BRAND_TEAM_DATA.sasha;
  const geoConfig =
    GEOGRAPHY_CONFIG[brandData.geo as keyof typeof GEOGRAPHY_CONFIG];
