...
export function TalentStreamModule() {
  ...
  return (
    ...
    >
      <div
        className="text-[9px] font-mono font-bold text-white/40 rotate-180 uppercase tracking-widest py-2"
      ...
      <div className="flex-1 w-full overflow-hidden relative flex flex-col items-center gap-2 mask-gradient-y">
        ...
        >
          {[...AVATARS, ...AVATARS].map((id, i) => (
            <div
              key={i}
              className="w-8 h-8 rounded-full bg-gradient-to-br from-zinc-700 to-zinc-600 border border-white/10 relative shrink-0"
            >
              {/* In a real app, use Image */}
              <div
                className={`absolute inset-0 rounded-full opacity-50 bg-indigo-500 mix-blend-overlay`}
              />
