                            ...
                            <div className="relative">
                                ...
                                <div className="bg-[#FDFBF7] dark:bg-zinc-900 rounded-[32px] shadow-2xl shadow-orange-500/5 border border-slate-100 dark:border-white/5 overflow-hidden p-6 sm:p-8">
                                    ...
                                    <div className="mb-8 p-4 bg-white dark:bg-zinc-800 rounded-2xl border border-slate-100 dark:border-zinc-700 shadow-sm">
                                        <div className="flex justify-between items-center mb-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Live Campaign</span>
                                            </div>
                                            <div className="text-xs font-medium text-slate-400">J+4 / 14</div>
                                        </div>

                                        {/* Campaign Timeline */}
                                        <div className="relative flex justify-between items-center">
                                            {/* Progress Bar Background */}
                                            <div className="absolute left-0 right-0 top-1.5 h-0.5 bg-slate-100 dark:bg-zinc-700 -z-10" />
                                                ...
                                                <div key={step.id} className="flex flex-col items-center gap-2">
                                                    <div className={`w-3 h-3 rounded-full border-2 ${step.status === 'completed' ? 'bg-green-500 border-green-500' :
                                                        step.status === 'active' ? 'bg-white border-green-500 ring-2 ring-green-100 dark:ring-green-900' :
                                                            'bg-slate-100 dark:bg-zinc-700 border-slate-200 dark:border-zinc-600'
                                                        }`} />
                                    ...
                                    <div className="flex justify-center mb-8">
                                        <div className="flex p-1 bg-slate-100 dark:bg-zinc-800 rounded-lg inline-flex">
                                            {TABS.map((tab) => (
                                                <button
                                                    key={tab.id}
                                                ...
                                                >
                                                    ...
                                                    {activeTab === tab.id && (
                                                        <motion.div
                                                            layoutId="activeTab"
                                                            className="absolute inset-0 bg-white dark:bg-zinc-700 rounded-md shadow-sm border border-slate-200 dark:border-zinc-600"
                            ...
                            <div className="pl-0 lg:pl-10">
                                ...
                                <div className="grid grid-cols-1 gap-5 mb-8">
                                    {TRACKING_FEATURES.map((feature) => (
                                        <div key={feature.id} className="group p-6 bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 hover:border-orange-500/30 transition-all duration-300 hover:shadow-xl hover:shadow-orange-500/5">
                                            ...
                                            <div className="mb-4">
                                                ...
                                                <ul className="space-y-1.5 pl-4">
                                                    {feature.elsewhere.map((item, j) => (
                                                        <li key={j} className="text-xs text-slate-400 dark:text-slate-500 leading-relaxed">
                                                            {item}
                                                        </li>
                                                    ))}
                                                </ul>
                                            ...
                                            <div className="p-4 bg-gradient-to-br from-orange-50 to-rose-50/50 dark:from-orange-500/5 dark:to-rose-500/5 rounded-xl border border-orange-100/50 dark:border-orange-500/10">
                                                ...
                                                <ul className="space-y-2">
                                                    {feature.wafia.map((item, j) => (
                                                        <li key={j} className="flex items-start gap-2 text-xs text-slate-700 dark:text-slate-300 font-medium leading-relaxed">
            ...
            <div className="bg-white dark:bg-zinc-800 rounded-2xl p-8 border border-slate-100 dark:border-zinc-700 shadow-sm relative overflow-hidden">
                <div className="flex justify-between items-start mb-8 relative z-10">
                    <div>
                        ...
                        <h2 className="text-6xl font-black text-slate-900 dark:text-white tracking-tighter">2.4M</h2>
                    </div>
                ...
                <div className="h-40 w-full relative z-10">
                    ...
                    {/* X-Axis Labels */}
                    <div className="flex justify-between text-[9px] font-medium text-slate-400 mt-2 uppercase">
                        <span>J1</span>
                        <span>J3</span>
                        <span>J5</span>
                        <span>J7</span>
...
function KpiCard({ label, value, trend, data, color }: { label: string, value: string, trend: string, data: number[], color: "orange" | "blue" }) {
    return (
        <div className="bg-white dark:bg-zinc-800 rounded-2xl p-5 border border-slate-100 dark:border-zinc-700 shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow">
    ...
    return (
        ...
        >
            {MOCK_CREATORS.map((creator, i) => (
                <div key={i} className="bg-white dark:bg-zinc-800 rounded-2xl p-4 border border-slate-100 dark:border-zinc-700 flex items-center justify-between hover:border-orange-200 dark:hover:border-orange-900 transition-colors group cursor-default shadow-sm">
                    <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-zinc-700 overflow-hidden relative border border-slate-200 dark:border-zinc-600">
                        ...
                        <div>
                            <p className="font-bold text-slate-900 dark:text-white text-sm">{creator.name}</p>
                            <p className="text-[10px] text-slate-400 font-medium uppercase tracking-wider">{creator.reach} reach</p>
                        </div>
                    </div>
                    <div className="text-right">
                        <div className="flex items-center gap-1 justify-end text-slate-900 dark:text-white font-bold text-base">
                            {creator.eng}
                            <TrendingUp className="w-3 h-3 text-green-500" />
                        </div>
                        <p className="text-[9px] text-slate-400 uppercase tracking-widest font-bold">AVG. ENG.</p>
                    </div>
                </div>
            ))}
            <div className="mt-6 text-center">
                <Button variant="ghost" size="sm" className="text-slate-400 hover:text-orange-600 text-xs font-medium uppercase tracking-wider">
...
function HistoryView() {
    return (
        <motion.div
            ...
            className="space-y-4"
        >
            <div className="bg-white dark:bg-zinc-800 rounded-2xl p-6 border border-slate-100 dark:border-zinc-700 shadow-sm">
                <div className="flex items-center justify-between mb-8">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Performances Cumulées</p>
