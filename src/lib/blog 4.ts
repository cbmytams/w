...
const TRUNCATED_DESCRIPTION_LENGTH = 157;
const FALLBACK_DESCRIPTION =
  "Analyse Wafia Knowledge sur l'influence, les plateformes et la monétisation des créateurs.";
...
function collapseAndNormalizeDescription(value: string): string {
  const normalized = value.replace(/\s+/g, " ").trim();
  if (!normalized) return FALLBACK_DESCRIPTION;
  if (normalized.length <= MAX_DESCRIPTION_LENGTH) return normalized;
  return `${normalized.slice(0, TRUNCATED_DESCRIPTION_LENGTH).trimEnd()}...`;
}
...
function normalizeDescription(
  ...
  const source = description || body || chapters[0]?.content || "";
  return collapseAndNormalizeDescription(source);
}
