...
export function validateQuestionnaireIntegrity(sectionsJson: unknown) {
  ...
  for (const section of normalizedSections || []) {
    if (!section.id || !section.title) {
      issues.push({
        ...
        message: `Section missing ID or title: ${JSON.stringify(section)}`,
      });
    }
    for (const q of section.questions || []) {
      if (!q.id) {
        issues.push({
          ...
          message: `Question without ID in section ${section.id}`,
        });
        continue;
      }
      if (questionIds.has(q.id)) {
        issues.push({
          type: "duplicate_id",
          message: `Duplicate question ID detected: ${q.id}`,
        });
      }
  ...
  const allQuestions = (normalizedSections || []).flatMap(
    (s) => s.questions || []
  );
      ...
      for (const condition of q.logic) {
        if (condition.jumpTo && condition.jumpTo !== "END") {
          if (!questionIds.has(condition.jumpTo)) {
            issues.push({
              type: "broken_jump",
              message: `Question ${q.id} jumps to non-existent ID: ${condition.jumpTo}`,
            });
          } else {
