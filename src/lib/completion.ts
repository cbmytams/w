...
const TALENTS_QUESTIONS_BY_ID = new Map(
  TALENTS_QUESTIONS.map((question) => [question.id, question])
);
const BRANDS_QUESTIONS_BY_ID = new Map(
  BRANDS_QUESTIONS.map((question) => [question.id, question])
);
...
export function computeCompletion(
  ...
  const questionsById =
    map.type === "BRANDS" ? BRANDS_QUESTIONS_BY_ID : TALENTS_QUESTIONS_BY_ID;
  ...
  if (requiredFields.length === 0)
    return { total: 0, answered: 0, percent: 100, missingFields: [] };
