...
export async function getFunnelSteps(filters: DashboardFilters): Promise<FunnelStep[]> {
    ...
    // Intentional mixed-date strategy:
    // each funnel stage is filtered on its own event timestamp (creation, submission, qualification, interview-ready),
    // so this view reflects stage activity in-period rather than a strict cohort flow from a single start date.
