...
export default async function BrandsSettingsPage() {
  const totalResponses = await prisma.questionnaireResponse.count({ where: { type: "BRANDS" } });
  const totalBrands = await prisma.talent.count({
    where: { questionnaireResponses: { some: { type: "BRANDS" } } },
  });
  const questionnaire = await prisma.questionnaire.findFirst({ where: { type: "BRANDS" } });

  const totalSections = BRANDS_QUESTIONNAIRE_MAP.sections.length;
  const totalFields = BRANDS_QUESTIONNAIRE_MAP.sections.reduce((s, sec) => s + sec.fields.length, 0);
  const requiredFields = BRANDS_QUESTIONNAIRE_MAP.sections.reduce((s, sec) => s + sec.fields.filter((f) => f.required).length, 0);

  const stats = [
    { icon: FileText, color: "text-violet-400", bg: "bg-violet-500/10", label: "Réponses", value: totalResponses },
    { icon: Shield, color: "text-blue-400", bg: "bg-blue-500/10", label: "Marques", value: totalBrands },
    { icon: Database, color: "text-orange-400", bg: "bg-orange-500/10", label: "Sections", value: totalSections },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="surface-card p-6">
        ...
        <h1 className="text-lg font-semibold text-white/90">Questionnaire Brands</h1>
      ...
      <div className="surface-card p-6">
        ...
        <div className="space-y-2">
          {BRANDS_QUESTIONNAIRE_MAP.sections.map((section, i) => (
