...
export default async function BrandsSettingsPage() {
  ...
  const totalFields = BRANDS_QUESTIONNAIRE_MAP.sections.reduce(
    (s, sec) => s + sec.fields.length,
    0
  );
  const requiredFields = BRANDS_QUESTIONNAIRE_MAP.sections.reduce(
    ...
    <div className="space-y-6">
      ...
      <div className="surface-card p-6">
        ...
        <p className="text-sm text-white/40 mt-1">
          Structure et statistiques du questionnaire en temps réel.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {stats.map(({ icon: Icon, color, bg, label, value }) => (
          <div key={label} className="surface-card p-5 flex items-center gap-4">
            ...
            <div>
              <div className="text-[10px] uppercase tracking-[0.25em] text-white/25 font-medium">
                {label}
              </div>
              <div className="text-2xl font-bold text-white/90 mt-0.5">
                {value}
              </div>
            </div>
          </div>
      ...
      {/* Structure */}
      <div className="surface-card p-6">
        <h2 className="text-xs uppercase tracking-[0.3em] text-white/30 font-medium mb-4">
      ...
      <div className="surface-card p-6">
        ...
        <div className="space-y-2">
          {[
            {
              label: "Version questionnaire",
              value: questionnaire ? `v${questionnaire.version}` : "—",
            },
