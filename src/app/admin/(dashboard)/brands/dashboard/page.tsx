...
export const metadata: Metadata = {
  ...
  description: "Vue d'ensemble des KPIs et activités — Portail Marques.",
};

export const dynamic = "force-dynamic";

const KPI_META = [
  { icon: Users, color: "from-violet-500 to-purple-600", shadow: "shadow-violet-500/20" },
  { icon: TrendingUp, color: "from-emerald-400 to-teal-500", shadow: "shadow-emerald-500/20" },
  { icon: FileCheck, color: "from-blue-400 to-indigo-500", shadow: "shadow-blue-500/20" },
  { icon: Clock, color: "from-orange-400 to-pink-500", shadow: "shadow-orange-500/20" },
];

export default async function BrandsDashboardPage(
  ...
  const totalBrands = await prisma.talent.count({
    where: { questionnaireResponses: { some: { type: "BRANDS" } } },
  });
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {kpis.slice(0, 4).map((kpi, i) => {
          ...
          const numVal = typeof kpi.value === "number" ? Math.round(kpi.value) : kpi.value;
          const unit = kpi.unit === "%" ? "%" : kpi.unit === "h" ? "h" : "";
          return (
            <div key={kpi.label} className="surface-card p-6 relative overflow-hidden group">
              <div className={`absolute right-4 top-4 w-10 h-10 rounded-xl bg-gradient-to-br ${meta.color} opacity-10 group-hover:opacity-20 transition-all duration-300 flex items-center justify-center`}>
                <Icon className="w-5 h-5 text-white" />
              </div>

              <div className="text-[10px] uppercase tracking-[0.3em] text-white/30 font-medium mb-3">{kpi.label}</div>

              <div className="text-4xl font-bold text-white/90 tracking-tight leading-none">
                {numVal}<span className="text-2xl font-medium text-white/50 ml-0.5">{unit}</span>
              </div>

              {kpi.delta !== 0 && (
                <div className={`mt-3 text-xs font-medium flex items-center gap-1 ${kpi.delta > 0 ? "text-emerald-400" : "text-red-400"}`}>
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="surface-card lg:col-span-2">
          <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-white/5">
            <h2 className="text-sm font-semibold text-white/80">Activité récente</h2>
            ...
            {recentResponses.length > 0 ? (
              recentResponses.map((r: typeof recentResponses[number]) => {
                ...
                return (
                  <Link
                    key={r.id}
                    href={`/admin/brands/questionnaires/${r.id}`}
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <div className="text-sm font-medium text-white/80 truncate group-hover:text-white transition-colors">{name}</div>
              ...
              <div className="py-16 text-center">
                ...
                <p className="text-xs text-white/20 mt-1">Les soumissions apparaîtront ici.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="surface-card p-6 flex flex-col gap-4">
          <h2 className="text-[10px] uppercase tracking-[0.3em] text-white/30 font-medium">Résumé</h2>

          <div className="space-y-3">
            {[
              { label: "Total marques", value: totalBrands },
          ...
          <div className="mt-auto pt-4 border-t border-white/5">
            <Link
              href="/admin/brands/questionnaires"
