...
export default async function BrandDetail({
  ...
  const companyName =
    (dataObj?.["ql_company"] as string) || "Entreprise Anonyme";
            ...
            {BRANDS_QUESTIONNAIRE_MAP.sections.map((section) => {
              ...
              return (
                <li key={section.id}>
                  ...
                  >
                    <span
                      className="text-sm text-slate-400 group-hover:text-white transition-colors truncate max-w-[180px]"
                      title={section.label}
                    >
                      {section.label}
                    </span>
                    <span
                      className={`text-xs font-medium ml-2 ${secComp.percent === 100 ? "text-green-500" : "text-questionnaire-primary"}`}
                    >
                      {secComp.percent}%
                    </span>
                  </a>
