...
const columns: ColumnDef<TalentListItem>[] = [
  {
    ...
    cell: (item) => (
      <div className="flex items-center gap-3">
        ...
        <span className="font-medium text-white">{item.name || "Anonyme"}</span>
      </div>
    ),
  },
  {
      ...
      return (
        <div className="flex items-center gap-2">
          <span className="text-xs text-slate-400">
            il y a{" "}
            {formatDistanceToNow(new Date(item.updatedAt), { locale: fr })}
          </span>
          {isNew && (
  ...
  },
  {
    ...
    cell: (item) => {
      ...
      return (
        <span
          className={`px-2 py-1 rounded text-[10px] uppercase tracking-wider font-semibold 
  ...
  },
  {
    ...
    cell: (item) => (
      <Link
        href={`/admin/talents/questionnaires/${item.id}`}
...
export function TalentListClient({ data }: { data: TalentListItem[] }) {
