...
export default async function BrandDetailPage({ params }: BrandDetailPageProps) {
  ...
  const answersData = (latestResponse?.answersJson as Record<string, unknown>) || {};
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="surface-card p-6">
          ...
          <div className="space-y-3 text-sm">
            ...
            {website && (
              <div className="flex items-center gap-3">
                <span className="w-4 h-4 text-muted shrink-0 text-center text-xs">🌐</span>
                <a href={website} target="_blank" rel="noreferrer" className="text-questionnaire-primary hover:underline truncate">{website}</a>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="surface-card p-6">
          <h2 className="text-xs uppercase tracking-[0.3em] text-soft mb-4">Informations clés</h2>
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between py-1 border-b border-black/5 dark:border-white/5">
              ...
              <span className="font-medium">{talent.createdAt.toLocaleDateString('fr-FR')}</span>
            </div>
            {latestResponse && (
              <div className="flex justify-between py-1">
                <span className="text-muted">Complétion questionnaire</span>
                <span className="font-semibold">{Math.round(latestResponse.completionRate)}%</span>
              </div>
            )}
          </div>
        </div>
