...
async function getQuestionnaire(version: string, type: QuestionnaireType) {
...
function createCSVStream(
    questionnaire: { id: string; version: string; sectionsJson: unknown },
    ...
    const stream = new ReadableStream({
        async start(controller) {
            try {
                ...
                const sectionsJson = questionnaire.sectionsJson as QuestionnaireSection[];
                ...
                const headers = ["ID", "Talent ID", "Score", "Completion Rate", "Submitted At", ...questionHeaders]
                    .map((header) => serializeCsvCell(header))
                    .join(",") + "\n";
                ...
                while (hasMore) {
                    ...
                    for (const response of chunkResponses) {
                        const answers = response.answersJson && typeof response.answersJson === "object"
                            ? (response.answersJson as Record<string, unknown>)
                            : {};

                        const row: string[] = [
                            serializeCsvCell(response.id),
                            serializeCsvCell(response.talentId),
                            serializeCsvCell(response.score ?? ""),
                            serializeCsvCell(response.completionRate),
                            serializeCsvCell(response.submittedAt.toISOString())
                        ];
    ...
    return new Response(stream, {
        headers: {
            ...
            "Content-Disposition": `attachment; filename="export-${type.toLowerCase()}-${questionnaire.version}-${new Date().toISOString().split('T')[0]}.csv"`,
...
function createJSONStream(
    ...
    return new Response(stream, {
        headers: {
            ...
            "Content-Disposition": `attachment; filename="export-${type.toLowerCase()}-${questionnaire.version}-${new Date().toISOString().split('T')[0]}.json"`,
            "Cache-Control": "no-cache",
            "Transfer-Encoding": "chunked"
        }
    });
...
export async function GET(request: NextRequest) {
    ...
    const questionnaire = await getQuestionnaire(version, type);
    if (!questionnaire) {
    ...
    if (format === "json") {
        return createJSONStream(questionnaire, type);
    }

    return createCSVStream(questionnaire, type);
}
